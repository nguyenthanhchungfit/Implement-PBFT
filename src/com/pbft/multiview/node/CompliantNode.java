...
import com.pbft.multiview.common.MessageUtils;
...
public class CompliantNode extends ConsensusNode {
	...
	public CompliantNode(String address, int port, int nodeId, KeyPair keyPair, IProcessor processor, Logger logger) {
		...
		this.logger = logger;
		this.server = new GMultiViewConsensusServer(port, processor);
		this.executor = new ThreadPoolExecutor(8, 8, 0L,
				java.util.concurrent.TimeUnit.MILLISECONDS, new java.util.concurrent.LinkedBlockingQueue<Runnable>());
	}
	...
	public void broadcastVoteMessage(GVoteMessage msg) {
		for (NeighborNode node : this.getNeighborNodes()) {
			this.getExecutor().execute(new Runnable() {
				@Override
				public void run() {
					node.sendVoteMessage(msg);
				}
			});
		}
	}
