...
public class GTendermintConsensusClient {
	private static final Logger LOGGER = LogManager.getLogger(GTendermintConsensusClient.class.getName());
	...
	public GTendermintConsensusClient(Channel channel, long timeoutInMillis) {
	...
	public GResult sendPreVoteMessage(GPreVoteMessage message) {
		try {
			Deadline deadline = Deadline.after(this.timeout, TimeUnit.MILLISECONDS);
			return this.blockingStub.withDeadline(deadline).onPreVoteMessage(message);
		} catch (StatusRuntimeException ex) {
			int error = _convertCustomError(ex.getStatus());
			return GResult.newBuilder().setError(error).build();
		} catch (Exception ex) {
	...
	public GResult sendPreCommitMessage(GPreCommitMessage message) {
		try {
			Deadline deadline = Deadline.after(this.timeout, TimeUnit.MILLISECONDS);
			return this.blockingStub.withDeadline(deadline)
					.onPreCommitMessage(message);
		} catch (StatusRuntimeException ex) {
			int error = _convertCustomError(ex.getStatus());
			return GResult.newBuilder().setError(error).build();
		} catch (Exception ex) {
